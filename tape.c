...
unsigned int send_tape_block (unsigned int offset) {
	...
	unsigned char i,r;
		...
		sprintf_P((char*)atari_sector_buffer,PSTR("Bloque %u / %u "),offset/block_len+1,(FileInfo.vDisk->size-1)/block_len+1);
...
unsigned int send_FUJI_tape_block (unsigned int offset) {
	...
	while(gap--)
        ...
        if (offset < FileInfo.vDisk->size) {	//data record
		print_str(115,215,2,Yellow,window_bg, "          ");
		sprintf_P((char*)atari_sector_buffer,PSTR("Bloque %u"),block);
		print_str(105,215,2,Yellow,window_bg, atari_sector_buffer);
		//read block
