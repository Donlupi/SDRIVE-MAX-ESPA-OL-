...
const char system_version[] PROGMEM = "para Chile " STR(SWVERSIONMAJOR) "." STR(SWVERSIONMINOR);
const char protector_pantalla[] PROGMEM = "ATARI CHILE";
const char system_info[] PROGMEM = "SDrive" STR(SWVERSIONMAJOR) STR(SWVERSIONMINOR) " " STR(DATE) "para Chile"; //SDriveVersion info

#define DISPLAY_IDLE 4000000
...
u08 newFile (u32 size) {	// size without ATR header
	...
	if(sec) {	// write ATR-Header
		...
		//blink(2);
		outbox_P(PSTR("error: creando nuevo"));
		return(1);
	}
}
...
ISR(TIMER2_OVF_vect) {
	...
	if (blank_count % 64 == 0) {
		...
		unsigned int col = rand() % 7;

		TFT_fill(Black);
		
		if (col == 0) {
			print_str_P(x,y,2,Yellow,Black, protector_pantalla);		
		}

		if (col == 1) {
			print_str_P(x,y,2,Orange,Black, protector_pantalla);		
		}
		...
		if (col == 3) {
			print_str_P(x,y,2,Blue,Black, protector_pantalla);		
		}

		if (col == 4) {
			print_str_P(x,y,2,Green,Black, protector_pantalla);		
		}

		if (col == 5) {
			print_str_P(x,y,2,White,Black, protector_pantalla);		
		}

		if (col == 6) {
			print_str_P(x,y,2,Red,Black, protector_pantalla);		
		}

		if (col == 7) {
			print_str_P(x,y,2,Grey,Black, protector_pantalla);		
		}
...
*/
	...
	if (r) {				// error on sd-card init
		sprintf_P((char*)atari_sector_buffer, PSTR("Error con tarjeta: %u"), r);
	...
	if (r)
	{
		sprintf_P((char*)atari_sector_buffer, PSTR("Error con FAT: %u"), r);
		outbox((char*)atari_sector_buffer);
		goto ST_IDLE;
	}
...
*/
		outbox_P(PSTR("No se encuentra SDRIVE.ATR"));
		//goto SD_CARD_EJECTED;
	}
			...
			if (b) {
				...
				if(actual_page == PAGE_MAIN && name[0] == 'N' &&
				   actual_drive_number != 0) {
					...
					name = pgm_read_ptr(&b->name);
					strncpy_P(&name[3], PSTR(">Nuevo_ATR<       "), 12);
					draw_Buttons();
				}
				//tape mode?
				if(actual_page == PAGE_TAPE && name[0] == 'I') {
					...
					if(tape_flags.run || pause->selected) {	//Stop
						...
						pause->selected = 0;
						print_str_P(35,135,2,Yellow,window_bg, PSTR("Detenido..   "));
						draw_Buttons();
					}
					else {		//Start
						FileInfo.vDisk->current_cluster=FileInfo.vDisk->start_cluster;
						...
						tape_flags.run = 1;
						flags->selected = 1;
						print_str_P(115,215,2,Yellow,window_bg, PSTR("[RETURN]"));
						draw_Buttons();
						if(!tape_flags.FUJI)
...
void process_command ()
{
	...
	if(!cmd_buf.cmd)
	{
		...
		if (err==0x02) {
			outbox_P(PSTR("SIO:CMD Tiempo excedido"));
		...
		{
			if (debug) {
				sprintf_P(atari_sector_buffer, PSTR("Error de SIO: %i"), err);
...
format_medium:
				...
				//and set FLAGS_WRITEERROR on error
				{
				 ...
				 if (formaterror)
				 {
				  ...
				  //blink(3);
				  outbox_P(PSTR("error: de escritura post format"));
				 }
				}
...
*/
		...
		case 0xE2: //$E2  n ??	Deactivation of device Dn:
			...
			strncpy_P(&name[3], PSTR("<vacio>     "), 12);
			set_display(actual_drive_number);
			goto Send_CMPL_and_Delay;
			break;
