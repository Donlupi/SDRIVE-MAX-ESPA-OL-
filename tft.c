...
const char traducido_str[] PROGMEM = "TRADUCCION POR DONLUPI";
const char linea_str[] PROGMEM = " ";
...
unsigned int list_files () {
	...
	sprintf_P(atari_sector_buffer, PSTR("%i archivos, pagina %i/%i  %s            "),
...
const struct button PROGMEM buttons_main[] = {
	...
	{"D1:<vacio>     ",10,40,240-21,30,Grey,Black,Black,&(struct b_flags){ROUND,1,0},action_b1_4},
	{"D2:<vacio>     ",10,80,240-21,30,Grey,Black,Black,&(struct b_flags){ROUND,1,0},action_b1_4},
	{"D3:<vacio>     ",10,120,240-21,30,Grey,Black,Black,&(struct b_flags){ROUND,1,0},action_b1_4},
	{"D4:<vacio>     ",10,160,240-21,30,Grey,Black,Black,&(struct b_flags){ROUND,1,0},action_b1_4},
	{"Caset:",66,200,80,30,Grey,Black,Black,&(struct b_flags){ROUND,1,0},action_tape},
	{"Nuevo",213-61,200,76,30,Grey,Black,Green,&(struct b_flags){ROUND,1,0},press},
	{"Opciones",115-61,240,110,30,Grey,Black,Blue,&(struct b_flags){ROUND,1,0},action_cfg},
	{"Outbox",10,280,240-11,320-1,0,0,0,&(struct b_flags){0,0,0},debug_page}
};

const struct button PROGMEM buttons_file[] = {
	{"Top",164,80,60,30,Grey,Black,White,&(struct b_flags){ROUND,1,0},list_files_top},
	{"Sube",164,120,60,30,Grey,Black,White,&(struct b_flags){ROUND,1,0},list_files_rev},
	{"Baja",164,160,60,30,Grey,Black,White,&(struct b_flags){ROUND,1,0},list_files},	
	{"Ultm",164,200,60,30,Grey,Black,White,&(struct b_flags){ROUND,1,0},list_files_last},
	{"OK!",164,240,60,30,Grey,Black,Green,&(struct b_flags){ROUND,1,0},action_ok},
	{"x",199,43,26,25,Red,Black,White,&(struct b_flags){ROUND,1,0},action_cancel},
	{"Archivo",15,45,150,240,Grey,Black,White,&(struct b_flags){ROUND,0,0},action_select}
};

//keep the order analog to struct tft.cfg, otherwise read/write function
//wouldn't work correctly!!!
const struct button PROGMEM buttons_cfg[] = {
	{"Rotar",15,45,90,30,Grey,Black,Light_Blue,&(struct b_flags){ROUND,1,0},action_change},
	...
	{"Salva-Pantalla",15,205,185,30,Grey,Black,Light_Blue,&(struct b_flags){ROUND,1,0},action_change},
	//!!leave this buttons at the end, then we can loop thru the previous!!
	{"Fijar Diskettes",15,245,200,30,Grey,Black,Light_Blue,&(struct b_flags){ROUND,1,0},action_change},
	{"Guardar",123,125,99,30,Grey,Black,Green,&(struct b_flags){ROUND,1,0},action_save_cfg},
	{"x",199,43,26,25,Red,Black,White,&(struct b_flags){ROUND,1,0},action_cancel}
};

const struct button PROGMEM buttons_tape[] = {
	{"Iniciar",15,165,100,30,Grey,Black,White,&(struct b_flags){ROUND,1,0},press},
	{"Pausa",15,205,80,30,Grey,Black,White,&(struct b_flags){ROUND,1,0},action_tape_pause},
	{"Turbo",144,165,80,30,Grey,Black,White,&(struct b_flags){ROUND,1,0},action_tape_turbo},
	{"x",205,103,26,25,Red,Black,White,&(struct b_flags){ROUND,1,0},action_cancel}
};
...
void main_page () {
	...
	print_str_P(150, 18, 1, White, Black, system_version); //"by KBr V1.0"
...
void tape_page () {
	...
	print_str_P(35, 135, 2, Orange, window_bg, PSTR("Modo Casetera"));
	Draw_H_Line(7,tft.width-8,154,Orange);
	draw_Buttons();
}
...
unsigned int debug_page () {
	...
	outbox_P(ready_str);
	outbox_P(linea_str);
	outbox_P(traducido_str);
	print_char(10,34,1,White,atari_bg,0x80);
